...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET/POST | `/factory-tool/login/` | 登录页 |
| GET | `/factory-tool/logout/` | 退出登录并清除会话 |
| GET | `/factory-tool/dashboard/` | Seller 与产品信息页 |
| GET | `/factory-tool/operations/` | DID 操作页 |
| GET | `/factory-tool/api/variants/` | 获取一级 Seller 的 Variant |
| GET | `/factory-tool/api/sub-sellers/` | 获取二级 Seller 列表 |
| GET | `/factory-tool/api/sub-sellers/<id>/variants/` | 获取二级 Seller 的 Variant |
| GET | `/factory-tool/api/query-did/?device_id=<did>` | 查询 DID 状态 |
| POST | `/factory-tool/api/bind-did/` | 绑定或更新单个 DID |
| POST | `/factory-tool/api/batch-bind/` | 服务端批量处理 |
